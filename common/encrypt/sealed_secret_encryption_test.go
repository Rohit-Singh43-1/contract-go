...
func TestEncryptSecretNative(t *testing.T) {
	...
	t.Logf("Generated sealed secret: %s", sealed[:100]+"...")
}
...
-----END PRIVATE KEY-----`)
	...
	verificationKey := []byte(`-----BEGIN PUBLIC KEY-----

awIDAQAB
-----END PUBLIC KEY-----`)
	...
	t.Logf("\n=== SEALED SECRET (Native Go Crypto) ===\n%s\n", sealed)
	t.Logf("\n=== USE THESE KEYS FOR DECRYPTION ===")
	t.Logf("SECRET_DECRYPTION_KEY:\n%s", string(decryptionKey))
	t.Logf("\nSECRET_VERIFICATION_KEY:\n%s", string(verificationKey))
